...
registerClientesController.register = async (req, res) => {
  ...
  try {
    ...
    const newCliente = new clientesModel({nombre, correo, contrasena:passwordHash, telefono, direccion, activo});
    ...
    res.json({
      message: "Cliente registrado, revise su correo con el código enviado",
    });
  } catch (error) {
